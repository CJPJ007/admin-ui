...
export default function Agents() {
  ...
  const fetchTestimonials = async () => {
    try {
      ...
      const data = await (
        await api(
          `/api/fieldSearch/advancedSearch/Testimonial?page=${currentPage}&size=${itemsPerPage}`,
          {
            method: "POST",
            headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditing && formData.id && formData.id >= 0) {
        await api(`/api/admin/testimonials/${formData.id}`, {
          method: "PUT",
  ...
  const handleEdit = (agent: Testimonial) => {
    setFormData(agent);
    setIsEditing(true);
